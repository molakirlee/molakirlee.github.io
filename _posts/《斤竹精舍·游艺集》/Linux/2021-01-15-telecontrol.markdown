...
1. 通过xshell+xmanager可以实现桌面的可视化（具体参见《Xshell显示图形化界面》），这个可视化实际上是个转发，即把服务器端弹出的可视化程序转发到控制端。此方法要求双向互通，否则信号回不来（比如某些在防火墙内的服务器，自己没有公网ipv4/ipv6地址，虽然直接访问不到，但能通过跳板机访问到；但关键是它想直接出来也出不来，所以实现不了信号转发。）。
