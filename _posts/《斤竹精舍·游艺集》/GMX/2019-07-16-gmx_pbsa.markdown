...
但是GMXPBSA有一些问题，比如gmxpbsa给的mdp太老会报错。今更新一下GMXPBSA的安装步骤：  
...
function PRINT_MINfile_n {
 echo -e "title = Energy in gas /ncpp   = /usr/bin/cpp /nconstraints = none /nnsteps = 1000 /nnstlist = 1 /nns_type = grid
rlist = 1 /nrcoulomb = 1 /nrvdw = 1 /npbc = xyz /n;" >> Mm.mdp
}
```
...
1. 通过 `use_topology` 允许使用自定义拓扑，注意要把top文件和其中包括的itp文件都一起放在根目录里；
1. 应该也可以通过修改力场文件实现上一步，加原子类型与电荷,但是自己修改不成功；
1. 把要分析的两个片段的拓扑文件（top或itp）提取成itp后分别命名为itp_receptor和itp_ligand；
1. ffield我选的amber5力场。
...
玺洛克修订版的GMXPBSA参见：[链接](molakirlee.github.io/attachment/gmx/GMXPBSA_XiLock.rar)


### g_MMPBSA

参考1:[入门教程](http://bioms.org/forum.php?mod=viewthread&tid=1240&extra=)    
参考1:[详细教程](http://www.bioms.org/forum.php?mod=viewthread&tid=1046)  
