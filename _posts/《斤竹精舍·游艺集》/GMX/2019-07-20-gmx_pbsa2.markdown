---
layout:     post
title:      "李老师计算MMPBSA的新脚本——gmxmmpbsa使用说明"
subtitle:   ""
...
### 前言
还是很长时间之前，我曾研究过gmx中的mmPBSA的实现方法。当时我发现有两种解决方式：gmxpbsa脚本与g_mmpbsa程序。粗略看了下，虽然两个程序都还算可用，但也不尽美，所以就没太关注了。

等到真正要做MMPBSA测试并试用这两种方法时，才发现它们使用起来并不顺利。gmxpbsa对gmx版本有要求且脚本写的很乱，出错了不容易调试；g_mmpbsa安装麻烦，二进制包，windows系统下无法使用，新版g_mmpbsa兼容gmx 5.1.x但不知是否兼容更新版本。基于以上原因，我萌生了自己改进gmxpbsa脚本之意。改着改着就几乎整个重写了，这就成了gmxmmPBSA脚本。这个过程加深了我对MMPBSA方法的理解，同时我也发现了存在的许多问题。

gmxMMPBSA使用时会调用gmx和APBS并可以支持任意版本，因此你可以安装最新版本而无需担心不支持。gmx只使用了trjconv和dump程序，前者用于处理轨迹的周期性叠合问题，后者用于抽取tpr中的原子参数。既然你已经有了轨迹文件说明肯定可以使用gmx，至于APBS则需要单独安装。
