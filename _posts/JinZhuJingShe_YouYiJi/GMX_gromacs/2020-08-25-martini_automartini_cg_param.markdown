---
layout:     post
title:      "gmx Martini力场粗粒化-实操auto_martini/cg_param/Automartini_M3"
subtitle:   ""
...
### [Automartini_M3](https://github.com/Martini-Force-Field-Initiative/Automartini_M3)
安装：
```
...
**注意**：win系统可能会提示找不到`BaseFeatures.fdef`，[下载](https://github.com/rdkit/rdkit/blob/master/Data/BaseFeatures.fdef)后放到提示的路径下即可。


### [cg_param](https://github.com/cgkmw-durham/cg_param_m3)
...
使用指令：
```
python cg_param_m3.py -s "[SMILES]" -f [NAME]
```

**注意**：要把cg_param_m3.py的1359行的`angles.append([x,int(shared),z])`改为`angles.append([x,int(shared.item()),z])`，因为`shared`里面是两个键共用的原子，只有一个，但数组不能直接整数化，所以要取第一个数后再整数化。
