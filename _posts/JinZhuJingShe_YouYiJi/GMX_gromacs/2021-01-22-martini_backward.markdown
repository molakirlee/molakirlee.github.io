...
###### Backward
1. linux系统下成功，win系统下的anaconda中会有路径问题，要做以下调整：1）进入pymol27等python 2的conda环境；2）[此条未必，若提示找不到grompp再改]将initram.sh`grompp`和`mdrun`分别改为`gmx grompp`和`gmx mdrun`;3)解决backward.py中Tab/空格混用问题，使用“查找/替换”功能将所有“\t”替换为8个空格.
1. `bash initram.sh -f md.gro -o aa_amber.gro -from martini -to amber -p topol.top` (eg. `bash initram.sh -f HCR-protein.pdb -o aa_charmm.gro -from martini -to charmm36 -p topol.top`)
1. 若原子重叠导致最小化失败，则自己编程让原子分开。
