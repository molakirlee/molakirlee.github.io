...
**几处问题待解决**：
1. 如果是在图形界面运行`vncserver :0 -localhost no -geometry 1920x1080 -depth 24`指令，则vnc端有桌面、鼠标、图标，如果是通过ssh输入则有桌面、鼠标，没有图标。初步猜测可能是因为SSH启动时缺乏图形界面相关的环境（就像SSH不能直接运行带桌面窗口的程序），因为注意到在图形界面端输入指令时屏幕会像开机一样刷新一下。可能需要在SSH的控制端安装与桌面显示相关的补丁？？
