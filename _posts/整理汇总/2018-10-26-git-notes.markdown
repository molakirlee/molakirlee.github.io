...
author:     "XiLock"
header-img: "img/in-post/collect_summarize/2018-10-26-git-notes/post-bg-rwd.jpg"
header-mask: 0.3
catalog:    true
tags:
    - 整理汇总
...
注：
1. 用fetch取回后并不改变本地开发代码，如果想编辑的话用merge命令合并取回的远端分支到本地分支（即便是已关联的分支）。
...
# 注：
# 1.若是将<远程分支>和当前分支合并，则可省略冒号后面的<本地分支名>。
# 2.若当前分支与<远程分支>存在追踪关系，则可省略<远程分支名>。
# 3.如果当前分支只有一个追踪分支，<连远程主机名>都可以省略。
```
4.### git push 本地分支推送到远端
```
$ git push <远程主机名> <本地分支名>:<远程分支名>

# 注：
# 1.如果<远程分支>不存在，则会被新建。
# 2.如果省略<本地分支名>，则表示删除指定的远程分支，因为这等同于推送一个空的本地分支到远程分支。
# 3.如果当前分支与<远程分支>之间存在追踪关系，则<本地分支>和<远程分支>都可以省略。
# 4.如果当前分支只有一个追踪分支，那么<远程主机名>都可以省略。
```
...
### git reset （三步）退回
![](/img/in-post/collect_summarize/2018-10-26-git-notes/schem.jpg)
```
